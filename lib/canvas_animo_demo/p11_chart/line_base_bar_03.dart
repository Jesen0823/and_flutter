...
class _Chart03State extends State<Chart03> with SingleTickerProviderStateMixin {
    ...
    return Stack(
      ...
      children: [
        Container(
          ...
          child: CustomPaint(
            painter: Chart03Painter(repaint: _controller),
          ),
        ),
        Padding(
...
class Chart03Painter extends CustomPainter {
  final TextPainter _textPainter =
      TextPainter(textDirection: TextDirection.ltr);
  final List<double> yData = [88, 60, 78, 87, 40, 86];
  ...
  Chart03Painter({required this.repaint}) : super(repaint: repaint) {
  ...
  void collectPoints(Size size) {
    points.clear();
    for (int i = 0; i < xData.length; i++) {
      double dataHeight = -(yData[i] / maxData * (size.height - scaleHeight));
      points.add(Offset(xStep * i - xStep / 2, dataHeight));
    }
  }
  ...
  void drawLineChart(Canvas canvas) {
    ...
    pms.forEach((pm) {
      canvas.drawPath(pm.extractPath(0, pm.length * repaint.value),
